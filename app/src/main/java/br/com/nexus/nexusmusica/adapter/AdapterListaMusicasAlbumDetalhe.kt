...
class AdapterListaMusicasAlbumDetalhe(
    private val listaMusica: List<Musica>,
    private val reproduzirMusica: (Musica) ->  Unit
) : RecyclerView.Adapter<AdapterListaMusicasAlbumDetalhe.ViewHodel>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHodel {
        return ViewHodel(
            AdapterListaMusicasAlbunsDetalheBinding.inflate(
    ...
    override fun onBindViewHolder(holder: ViewHodel, position: Int) {
        ...
        holder.txtNumFaixaAlbum.text =
            if (numMusica.length >= 4) numMusica.slice(3..3) else numMusica
    ...
    inner class ViewHodel(binding: AdapterListaMusicasAlbunsDetalheBinding) :
